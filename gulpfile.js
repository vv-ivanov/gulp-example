...
var paths = {
    ...
    video: {
        src: options.srcDir + '/video',
        dest: options.destDir + '/video',
        mask: '/**/*.*'
    },
    watch: {
...
//если нужно отключить минификацию картинок, раскоментировадь код и закоментировать код выше
// gulp.task('assets:img', function () {
//     return gulp.src(paths.img.src + paths.img.mask)
//         .pipe(gulp.dest(paths.img.dest));
// });
...
gulp.task('assets:video', function () {
    return gulp.src(paths.video.src + paths.video.mask)
...
gulp.task('watch', ['build'], function () {
    ...
    gulp.watch(paths.watch.img, ['assets:img']);
    gulp.watch(paths.watch.video, ['assets:video']).on('change', function (event) {
        handleWatchEvent(event, paths.video, 'VIDEO')
    });
    if (options.useTemplates) {
